...
------------------------------------------
    h_int(7) <= to_signed(G_H_0,h_int(7)'length);
    h_int(6) <= to_signed(G_H_1,h_int(6)'length);
    h_int(5) <= to_signed(G_H_2,h_int(5)'length);
    h_int(4) <= to_signed(G_H_3,h_int(4)'length);
    h_int(3) <= to_signed(G_H_4,h_int(3)'length);
    h_int(2) <= to_signed(G_H_5,h_int(2)'length);
    h_int(1) <= to_signed(G_H_6,h_int(1)'length);
    h_int(0) <= to_signed(G_H_7,h_int(0)'length);
    ...
    begin
        if rising_edge(i_clk) then
            if RESET_G = '1' or i_fen = '0' then
                ...
                if i_cen = '1' and i_fen = '1' then
                    for i in 0 to 7 loop
                        mult_int(i) <= resize((cos_int(i) * h_int(7-i)), mult_int(i)'length);
                    end loop;
                end if;
            end if;
        end if;
    ...
    begin
        if rising_edge(i_clk) then
            if RESET_G = '1' or i_fen = '0' then
                ...
                if i_cen = '1' and i_fen = '1' then
                    add_int(0)  <= resize(mult_int(0)+"000000000000000", add_int(0)'length);
                    for i in 0 to 6 loop
                        add_int(i+1)   <= mult_int(i+1)+add_int(i);
                    end loop;
                end if;
            end if; 
        end if;
